...
Every number in Quiver is a `Double`: a 64-bit floating-point value that approximates a real number using a finite pattern of bits. Most of the time, that approximation is close enough that we can ignore it. The mean of a column, the dot product of two vectors, the determinant of a matrix all compute to results that match what we would get with pencil and paper to as many decimal places as anyone reasonably needs.
...
> Tip: Compare floating-point values with a tolerance: `abs(a - b) < 1e-9` is a reliable default. The `1e-9` is Swift's shorthand for `0.000000001`, where the `e` reads as "times ten to the," so `1e-9` is one billionth. Quiver follows this rule internally and we follow it in any code that consumes Quiver's output.
