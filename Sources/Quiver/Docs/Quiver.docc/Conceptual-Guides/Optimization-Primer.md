...
The algorithm introduced here for regression is the one the classification models need too. Each arrives mainly by changing the loss the loop is handed, not by rewriting the loop. ``LogisticRegression`` predicts probabilities by wrapping its linear output in a squashing function and minimizing a loss suited to probabilities rather than squared error. Because that loss has no closed-form answer either, the iterative algorithm is not a convenience but the only route. The same is true of the margin-maximizing classifiers that will follow. The loop a reader learns on a straight line is the loop that fits all of them.
