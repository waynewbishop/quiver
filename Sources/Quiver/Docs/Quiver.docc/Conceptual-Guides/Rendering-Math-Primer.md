...
Two boundary behaviors complete the picture. The number formatter switches from `%.4f` to `%.3g` at three significant figures when `0 < |x| < 1e-3`, so a coefficient of `1e-5` renders as `"1e-05"` rather than collapsing to `"0.0000"`. The switchover protects distinct sub-millisecond values from rounding to the same `"0"` cell. It also produces one rendering quirk worth knowing about. Values just below `1e-3` stay in decimal form under `%.3g`, so `9e-4` renders as `"0.0009"` and `0.0009999` rounds up to `"0.001"`, the same string the value `0.001` itself produces through the `%.4f` branch. Two coefficients differing by a tenth of a percent can land on the same rendered string. The behavior is correct under the rule, but it is worth knowing exists before it surprises a fitted result. The second boundary is at the integer line. Integer-valued doubles render without a decimal point, so `5` appears rather than `5.0000`, but a value like `0.99999` keeps its full `%.4f` form (`"1.0000"`) rather than rounding-and-trimming to `"1"`, because advertising a non-integer as an integer would lie about what the underlying `Double` holds.

A consequence of the formatting worth flagging once: the rendered string is not a round-trip-safe encoding of the underlying `Double`. Parsing `"0.3846"` back to a floating-point value will not recover the original `5.0 / 13.0` bit-for-bit. The string is for the reader; the `Double` is for the computation. Likewise, `Fraction(0.1)` returns `1/10` even though the `Double` value `0.1` is not exactly one-tenth; it is the closest representable binary fraction. `Fraction` is a teaching surface, not an exact-arithmetic system. Both methods change the form of a result, never the result itself; the underlying `Double` values are untouched.
