...
Ordinary least squares, shown above, yields a lopsided pair. If we push the collinearity further, to two columns that are exact multiples, the matrix inversion fails, and `LinearRegression` throws `MatrixError.singular`. This refusal is honest: with no unique solution, the model provides none.
...
This dial requires a gauge. A small `lambda` leaves the model free to overfit; a large `lambda` shrinks weights so far the model underfits, growing too rigid to follow the real pattern. Overfitting and underfitting are not separate problems to solve; they are opposite ends of one dial. The art lies not in eliminating failure, but in finding the setting where the model generalizes best.
