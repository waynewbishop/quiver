...
- **Naive Bayes** for classification, especially when features are roughly independent. This is *statistical* independence — features contributing information separately given the label — not the *linear* independence of columns that regression needs; <doc:Determinants-Primer> pulls the two senses apart.
