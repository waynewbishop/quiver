...
A **True Effort Score** keeps all six dimensions in play. It reads effort from heart rate, pace, cadence, grade, running power, and vertical oscillation together — the full signal the body produces in each moment — rather than from any one of them alone. A heart rate inflated by heat, by a climb, or by a fast descent looks the same on its own, but sits in a different place once the other five signals are present, and the model can tell the cases apart.

### Reading the full signal

The way it reads that full signal is to treat heart rate as a downstream response to workload rather than a measure of it, and to predict the heart rate the other signals say a moment should have produced. The gap between that expectation and the observed heart rate — the **residual** — is where the information a single-signal score throws away comes back. That gap has a name in exercise science: **cardiac decoupling**, when heart rate and the workload that drives it stop moving together. Heart rate drifts up while the work holds steady, or stays high while the work drops away — and that divergence is exactly what a single-signal score cannot see.

### The models that do the work

Three Quiver models do the work, each with a distinct job. A ``StandardScaler`` puts the signals on one common scale, so no single dimension dominates the others by its raw units. A ``Ridge`` regression is the baseline: it predicts the expected heart rate from the workload signals — pace, cadence, grade, vertical oscillation, and altitude — and the residual against the observed heart rate is the effort signal itself. Running power is held out of this fit: it is so nearly a restatement of pace and grade that including all three makes the regression unstable, a point the baseline section returns to. A ``KNearestNeighbors`` classifier then labels the moment by how it resembles past efforts, catching the abrupt cases the residual cannot — the descent where heart rate stays high while the work falls away. The classifier rides inside a ``Pipeline`` with its own scaler, so the two are always applied together.
...
Picture a runner tipping over the lip of a steep descent. They let gravity take over — turnover quickens, breathing stays relaxed, they are barely working the engine — yet the heart rate is still sitting at `160`, right where it was on the climb above. A heart-rate-only score reads that `160` and calls the stretch hard. It is wrong about the cardiovascular side: the heart is not working hard here, it is coasting down off the climb. But the descent is not free either. Every downhill stride lands with the quadriceps lengthening under load — eccentric loading, the largest single source of next-day soreness in a runner's week. The single number gets it doubly wrong: it overstates the cardiovascular demand while missing entirely that the legs are taking the heaviest mechanical hit of the run. The label "hard" and the label "easy" are both true, of different systems, and one number cannot hold both.

The same runner then hits a wall of a climb, too steep to run, and drops to a power-hike — hands on the quads, driving up one big step at a time. Pace falls off a cliff, toward eighteen or twenty minutes per mile, and cadence collapses into a slow, weighted rhythm. A pace- or cadence-based score sees that crawl and calls it easy, practically a rest. That is badly wrong in the other direction: lifting bodyweight up a steep grade is genuinely hard work, and here the heart rate reflects it honestly. This is the cleaner case — uphill, the heart rate tells close to the truth, because cardiovascular and muscular demand both rise and roughly agree. The signal lying now is pace, which reads slow and concludes easy while the body grinds out some of the hardest work of the session.

The problem does not even need terrain. Hold a steady, conversational pace on a flat loop on a warm afternoon, and the heart rate still drifts ten or fifteen beats upward over the back half. The effort never changed — the cost of cooling did, as blood shunts to the skin and the heart beats faster to do the same job. The flat, unchanging pace is exactly the signal that says the rising heart rate is not a harder effort.
...
A wrist sensor produces far more than a heart rate. Each instant of a run is a six-signal snapshot — heart rate, pace, cadence, grade, running power, and vertical oscillation — a point in six-dimensional space describing what the body is doing right then. Reducing that point to one number is a projection, and a projection throws information away on purpose. Two efforts that look identical from one angle sit far apart in the full space: the downhill `160` and the uphill `160` are the same shadow cast by different shapes. The moment we keep all the dimensions, the two are no longer the same point, and a model can tell them apart.

This is where the framework earns its place. Quiver treats each moment as the whole vector it is — a plain `[Double]` of every signal at once — and the same array flows through scaling, regression, and classification without ever being flattened to a scalar first. Working in the full space rather than one axis at a time is the difference between a score that confuses a wrecked-quads descent with a threshold effort and one that holds them apart. The model below does not use every signal in every component — a coefficient earns its slot only when it carries information the others do not — but the starting point is always the full picture, never a single projection of it.
...
With an expected heart rate in hand, the effort signal is plain subtraction. The watch reports a heart rate of `162`. The baseline, given the same pace, cadence, grade, oscillation, and altitude, expected `152.9`. The residual is the difference:
...
That `9.1` beats per minute is heart rate the external workload does not explain — the signature of an inflating effect such as heat or drift. The sign carries meaning in both directions. A residual of `−7` would mean the heart is running cooler than the workload predicts, the structural blind spot a regression cannot see on its own: the fast, low-demand descent where the cardiovascular demand is low even though pace is high.

It is worth being exact about what the residual is and is not. It detects that decoupling is happening; it does not diagnose the cause. A large positive residual is consistent with heat, altitude, dehydration, or accumulating fatigue, and the model cannot separate them — it has a heart rate and a workload, not a thermometer. Naming a single cause would claim more than the signal supports.
...
The classifier needs labeled examples, but a runner who has just installed the app has labeled nothing. A ``KMeans`` pass solves the cold start. Run unsupervised over a runner's first few sessions, it groups the moments into clusters by their kinematic shape, and the clusters line up with effort levels: rank them by their average heart rate and pace, and the highest-intensity cluster is Hard, the lowest is Easy. Those cluster assignments become the provisional labels the classifier trains on, so a usable model exists after the first runs with no data entry. The labels sharpen later, as the runner confirms or corrects efforts and the classifier retrains on the corrected set.

## Reading the baseline as math

A regression that hides its reasoning is just another opaque score. Because the baseline is linear, we can render it as the equation it is. The `asExpression` method on the coefficients prints the intercept and each signal's weight:
...
> Experiment: **The Quiver Notebook** is the right place to watch the residual do its work. Build the baseline, then take one steady stretch of a run and add a slow upward drift to its heart-rate series — a few beats climbing over several minutes, as heat would produce. Re-read the residual and watch it absorb the drift while the effort label holds steady. The number the baseline could not explain is exactly the effect you injected. See <doc:Quiver-Notebook>.
